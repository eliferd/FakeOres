...
public class EntityOresBoss extends EntityMob implements IBossDisplayData
{
    ...
    public EntityOresBoss(World par1World)
    {
        ...
        this.setSize(3F, 5F);
        this.tasks.addTask(4, new AIAttackPhase());
    ...
    public void attackEntityWithRangedAttack(EntityLivingBase par1EntityLivingBase, float par2)
    {
        ...
        entityarrow.setThrowableHeading(d0, d1 + f1, d2, 1.6F, 12.0F);
    ...
    public void attackEntity(Entity entity, float f)
    {
        if(phase == 1)
        {
            ...
            if(rand.nextInt(24) == 0)
            {
                ...
                this.worldObj.playAuxSFXAtEntity((EntityPlayer)null, 1009, new BlockPos(this.posX, this.posY, this.posZ), 0);
                for(int i = 0; i < 1; ++i)
                {
                    EntityLargeFireball entitysmallfireball = new EntityLargeFireball(this.worldObj, this, d0 + this.rand.nextGaussian() * f1, d1, d2 + this.rand.nextGaussian() * f1);
    ...
    public boolean attackEntityFrom(DamageSource damagesource, float f)
    {
        ...
        {
            /*
             * ItemStack itemstack = ((EntityPlayer)entity).getCurrentEquippedItem(); if(itemstack != null) { if(itemstack.getItem() == Items.magma_cream) { f = 150f; } }
             */
    ...
    class AIAttackPhase extends EntityAIBase
    {
        EntityOresBoss oresBoss = EntityOresBoss.this;
        private int attackTime;

        @Override
        public boolean shouldExecute()
        {
            EntityLivingBase livingBase = this.oresBoss.getAttackTarget();
        ...
        public void updateTask()
        {
            --attackTime;
            EntityLivingBase livingBase = this.oresBoss.getAttackTarget();
            double d0 = this.oresBoss.getDistanceSqToEntity(livingBase);
            ...
            if(d0 < 4.0D)
            {
                ...
                {
                    this.attackTime = 20;
                    this.oresBoss.attackEntityAsMob(livingBase);
                }
                this.oresBoss.getMoveHelper().setMoveTo(livingBase.posX, livingBase.posY, livingBase.posZ, 1.0D);
            }
            else if(d0 < 256.0D)
            {
                this.oresBoss.attackEntity(livingBase, (float)d0);
                this.oresBoss.getLookHelper().setLookPositionWithEntity(livingBase, 10.0F, 10.0F);
            }
            else
            {
                this.oresBoss.getNavigator().clearPathEntity();
                this.oresBoss.getMoveHelper().setMoveTo(livingBase.posX, livingBase.posY, livingBase.posZ, 0.0D);
            }
        }
    }
}
...
public class EntityNopeGrenade extends EntityItem
{
    ...
    public EntityNopeGrenade(World world, EntityLivingBase entityliving)
    {
        ...
        motionX = initialVelocity * d * MathHelper.cos((entityliving.rotationPitch / 180F) * (float)Math.PI);
        motionY = -initialVelocity * MathHelper.sin((entityliving.rotationPitch / 180F) * (float)Math.PI);
        motionZ = initialVelocity * d1 * MathHelper.cos((entityliving.rotationPitch / 180F) * (float)Math.PI);
        ...
        setPosition(entityliving.posX + d * 0.8D, entityliving.posY + entityliving.getEyeHeight(), entityliving.posZ + d1 * 0.8D);
        prevPosX = posX;
        prevPosY = posY;
        prevPosZ = posZ;
    }
    ...
    public void onUpdate()
    {
        ...
        boolean flag = false;
        List list = worldObj.getEntitiesWithinAABB(EntityCreature.class, AxisAlignedBB.fromBounds(posX, posY, posZ, posX + 1, posY + 1, posZ + 1).expand(1D, 1D, 1D));
        for(int i = 0; i < list.size(); i++)
        {
            ...
            worldObj.spawnParticle(EnumParticleTypes.SMOKE_LARGE, posX + rand.nextInt(2), posY + rand.nextInt(2), posZ + rand.nextInt(2), 0.0D, 0.0D, 0.0D);
        }
    ...
    protected void part(World worldObj, EntityNopeGrenade entity)
    {
        for(int i = 0; i < 20; i++)
        {
            ...
            if(smokedTime > 0)
            {
                ...
                {
                    FakeOres.proxy.spawnParticle(worldObj, "nope", (entity.posX + rand.nextFloat() * entity.width * 2.0F) - entity.width - d * d3, (entity.posY + rand.nextFloat() * entity.height) - d1 * d3, (entity.posZ + rand.nextFloat() * entity.width * 2.0F) - entity.width - d2 * d3);
                }
            }

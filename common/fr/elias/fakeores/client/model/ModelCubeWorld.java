 ...
 */
/*
 * public void render() { glPushMatrix(); glDisable(GL_TEXTURE_2D); glEnable(GL_RESCALE_NORMAL); final float scale = 0.0625F; glScalef(scale, scale, scale); glRotatef(-90F, 1F, 0F, 0F); for(int i : pointers) { final int x = i & 1023; final int y = (i >> 10) & 1023; final int z = (i >> 20) & 1023; glTranslatef(x, y, z); final byte[] color = modelData[x][y][z]; glColor3ub(color[0], color[1], color[2]); cube.render(1F); glTranslatef(-x, -y, -z); } glEnable(GL_TEXTURE_2D); glDisable(GL_RESCALE_NORMAL); glPopMatrix(); } }
 */
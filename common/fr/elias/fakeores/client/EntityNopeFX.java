...
public class EntityNopeFX extends EntityFX
{
    ...
    public void func_180434_a(WorldRenderer renderer, Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
    {
        ...
        /*
         * Minecraft.getMinecraft().renderEngine.bindTexture(new ResourceLocation("fakeores:textures/items/nope.png")); GL11.glDepthMask(false); GL11.glEnable(GL11.GL_BLEND); GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA); GL11.glAlphaFunc(GL11.GL_GREATER, 0.003921569F); tessellator.startDrawingQuads(); tessellator.setBrightness(getBrightnessForRender(f)); float scale = 0.1F * particleScale; float x = (float)(prevPosX + (prevPosX - posX) * f - interpPosX), y = (float)(prevPosY + (prevPosY - posY) * f - interpPosY), z = (float)(prevPosZ + (prevPosZ - posZ) * f - interpPosZ); float f14 = getBrightness(f); tessellator.setTranslation(0F, 0F, 0F); tessellator.setColorOpaque_F(f14 * particleRed, f14 * particleGreen, f14 * particleBlue); tessellator.addVertexWithUV(x - f1 * scale - f4 * scale, y - f2 * scale, z - f3 * scale - f5 * scale, 1, 1); tessellator.addVertexWithUV(x - f1 * scale + f4 * scale, y + f2 * scale, z - f3 * scale + f5 * scale, 1, 0); tessellator.addVertexWithUV(x + f1 * scale + f4 * scale, y + f2 * scale, z + f3 * scale + f5 * scale, 0, 0); tessellator.addVertexWithUV(x + f1 * scale - f4 * scale, y - f2 * scale, z + f3 * scale - f5 * scale, 0, 1); tessellator.draw(); GL11.glDisable(GL11.GL_BLEND); GL11.glDepthMask(true); GL11.glAlphaFunc(GL11.GL_GREATER, 0.1F);
         */
    }

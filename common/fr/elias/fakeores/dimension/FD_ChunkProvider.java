...
public class FD_ChunkProvider implements IChunkProvider
{
	...
	public FD_ChunkProvider(World world, long seed, boolean features)
	{
		this.worldObj = world;
					...
					for(int l2 = 0; l2 < 8; ++l2)
					{
						...
						for(int i3 = 0; i3 < 4; ++i3)
						{
							...
							for(int k3 = 0; k3 < 4; ++k3)
							{
								if((d15 += d16) > 0.0D)
								{
									surfaceBlock[j3+=short1] = FakeOres.fd_stone; 
								}
								else if(k2 * 8 + l2 < b0)
								{
	...
	public void replaceBlocksForBiome(int x, int z, Block[] block, byte[] arrayOfByte, BiomeGenBase[] biomeList)
	{
		...
		for(int k = 0; k < 16; ++k)
		{
			for(int l = 0; l < 16; ++l)
			{
				BiomeGenOreDimension biomegenbase = (BiomeGenOreDimension)biomeList[l + k * 16];
				biomegenbase.generateTerrainBlocks(this.worldObj, this.rand, block, arrayOfByte, x * 16 + k, z * 16 + l, this.stoneNoise[l + k * 16]);
			}
		}
	}
	...
	{
		BiomeGenOreDimension dimCreature = (BiomeGenOreDimension)this.worldObj.getBiomeGenForCoords(x, z);

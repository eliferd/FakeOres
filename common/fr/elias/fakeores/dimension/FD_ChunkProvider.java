...
public class FD_ChunkProvider implements IChunkProvider
{
    private Random rand;
    ...
    public FD_ChunkProvider(World world, long seed, boolean features, String generatorSettings)
    {
        this.worldObj = world;
                    ...
                    for(int l2 = 0; l2 < 8; ++l2)
                    {
                        ...
                        for(int i3 = 0; i3 < 4; ++i3)
                        {
                            ...
                            for(int k3 = 0; k3 < 4; ++k3)
                            {
                                if((d15 += d16) > 0.0D)
                                {
                                    primer.setBlockState(k * 4 + i3, k2 * 8 + i2, j1 * 4 + j3, FakeOres.fd_stone.getDefaultState());
                                }
                                else if(k2 * 8 + l2 < this.settings.seaLevel)
                                {
                                    primer.setBlockState(k * 4 + i3, k2 * 8 + l2, j1 * 4 + j3, FakeOres.fd_strangeliquid.getDefaultState());
                                }
                            }
    ...
    public void func_180517_a(int x, int z, ChunkPrimer primer, BiomeGenBase[] biomes)
    {
        ...
        double d0 = 0.03125D;
        this.stoneNoise = this.noiseGen4.func_151599_a(this.stoneNoise, x * 16, z * 16, 16, 16, d0 * 2.0D, d0 * 2.0D, 1.0D);

        for(int k = 0; k < 16; ++k)
        {
            for(int l = 0; l < 16; ++l)
            {
                BiomeGenOreDimension biomegenbase = (BiomeGenOreDimension)biomes[l + k * 16];
            ...
            for(int k1 = 0; k1 < 5; ++k1)
            {
                ...
                for(int l1 = -b0; l1 <= b0; ++l1)
                {
                    for(int i2 = -b0; i2 <= b0; ++i2)
                    {
                        ...
                        float f3 = this.settings.biomeDepthOffSet + biomegenbase1.minHeight * this.settings.biomeDepthWeight;
                        float f4 = this.settings.biomeScaleOffset + biomegenbase1.maxHeight * this.settings.biomeScaleWeight;
                ...
                double d14 = f;
                d12 += d13 * 0.2D;
                d12 = d12 * this.settings.baseSize / 8.0D;
                double d5 = this.settings.baseSize + d12 * 4.0D;
    ...
    public void populate(IChunkProvider provider, int x, int z)
    {
        ...
        if(biomegenbase != BiomeGenBase.desert && biomegenbase != BiomeGenBase.desertHills && !this.settings.useWaterLakes && this.rand.nextInt(this.settings.waterLakeChance) == 0 && TerrainGen.populate(provider, worldObj, rand, x, z, flag, LAKE))
        {
            k1 = k + this.rand.nextInt(16) + 8;
            l1 = this.rand.nextInt(256);
            i2 = l + this.rand.nextInt(16) + 8;
            (new WorldGenLakes(Blocks.water)).generate(this.worldObj, this.rand, pos.add(k1, l1, i2));
        }
        if(this.settings.useDungeons)
        {
            ...
            for(k1 = 0; doGen && k1 < this.settings.dungeonChance; ++k1)
            {
                ...
                (new WorldGenDungeons()).generate(this.worldObj, this.rand, pos.add(l1, i2, j2));
            }
        }
        ...
        if(rand.nextInt(15) == 0)
        {
            for(k1 = 0; k1 < 8; ++k1)
            {
                ...
                i2 = this.rand.nextInt(256);
                int j2 = l + this.rand.nextInt(16) + 8;
                (new WorldGenMaze()).generate(this.worldObj, this.rand, pos.add(l1, i2, j2));
            }
        }
        ...
        boolean doGen = TerrainGen.populate(provider, worldObj, rand, x, z, flag, ICE);
        for(k1 = 0; doGen && k1 < 16; ++k1)
        {
            for(l1 = 0; l1 < 16; ++l1)
            {
                BlockPos pos1 = this.worldObj.getPrecipitationHeight(pos.add(k + k1, 0, l + l1));
                BlockPos pos2 = pos1.down();

                if(this.worldObj.func_175675_v(pos2))
                {
                    this.worldObj.setBlockState(pos2, Blocks.ice.getDefaultState(), 2);
                }

                if(this.worldObj.canSnowAt(pos1, true))
                {
                    this.worldObj.setBlockState(pos1, Blocks.snow_layer.getDefaultState(), 2);
                }
            }
        }
    ...
    {
        BiomeGenOreDimension dimCreature = (BiomeGenOreDimension)this.worldObj.getBiomeGenForCoords(pos);
        return creature == EnumCreatureType.MONSTER && this.scatteredFeatureGenerator.func_175798_a(pos) ? this.scatteredFeatureGenerator.getScatteredFeatureSpawnList() : dimCreature.getSpawnableList(creature);
    }

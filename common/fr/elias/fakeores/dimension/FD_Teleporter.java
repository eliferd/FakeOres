...
public class FD_Teleporter extends Teleporter
{
			...
			for(int l = -2; l <= 2; ++l)
			{
				for(int i1 = -2; i1 <= 2; ++i1)
				{
					for(int j1 = -1; j1 < 3; ++j1)
					{
						...
						this.worldServerInstance.setBlock(k1, l1, i2, flag ? FakeOres.fd_redobsidian : Blocks.air);
					}
				}
			}
	...
	public boolean placeInExistingPortal(Entity entity, double x, double y, double z, float rotationYaw)
	{
		...
		else
		{
			for(k1 = l - short1; k1 <= l + short1; ++k1)
			{
				...
				for(int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
				{
					...
					{
						if(this.worldServerInstance.getBlock(k1, i2, l1) == FakeOres.fd_portal)
						{
							while(this.worldServerInstance.getBlock(k1, i2 - 1, l1) == FakeOres.fd_portal)
							{
								--i2;
							}
		...
		if(d3 >= 0.0D)
		{
			if(flag)
			{
				this.destinationCoordinateCache.add(j1, new FD_PortalPosition(this, i, j, k, this.worldServerInstance.getTotalWorldTime()));
			...
			d4 = (double)k + 0.5D;
			int j2 = -1;
			if(this.worldServerInstance.getBlock(i - 1, j, k) == FakeOres.fd_portal)
			{
				j2 = 2;
			}
			if(this.worldServerInstance.getBlock(i + 1, j, k) == FakeOres.fd_portal)
			...
			if(this.worldServerInstance.getBlock(i, j, k - 1) == FakeOres.fd_portal)
			{
				j2 = 3;
			}
			if(this.worldServerInstance.getBlock(i, j, k + 1) == FakeOres.fd_portal)
			...
			for(k2 = -1; k2 <= 1; ++k2)
			{
				for(i3 = 1; i3 < 3; ++i3)
				{
					for(l2 = -1; l2 < 3; ++l2)
					{
						...
						this.worldServerInstance.setBlock(k3, j3, i4, flag ? FakeOres.fd_redobsidian : Blocks.air);
					}
				}
			}
		}
		for(k2 = 0; k2 < 4; ++k2)
		{
			for(i3 = 0; i3 < 4; ++i3)
			{
				for(l2 = -1; l2 < 4; ++l2)
				{
					...
					this.worldServerInstance.setBlock(k3, j3, i4, flag ? FakeOres.fd_redobsidian : FakeOres.fd_portal, 0, 2);
				}
			}
			for(i3 = 0; i3 < 4; ++i3)
			{
